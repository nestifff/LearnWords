package com.nestifff.learnwords.presentation.ui.theme.values.colors

import androidx.compose.ui.graphics.Color

val wordsLightPalette = ColorCommon(
    // region Base
    background = Color(0xFFFFFFFF),
    backgroundLight = Color(0xFFF7F7F7),
    backgroundMedium = Color(0xFFB6B6B6),
    ripple = Color(0xFF858585),
    icons = Color(0xFF1A1A1A),
    primary = Color(0xFF3DD642),
    primaryLight = Color(0xFFA6FFA9),
    text = Color(0xFF1A1A1A),
    textLight = Color(0xFF5F5F5F),
    expandedDialogBackground = Color(0x331A1A1A),
    // endregion
)

val wordsDarkPalette = ColorCommon(
    // region Base
    background = Color(0xFF343434),
    backgroundLight = Color(0xFFE4E4E4),
    backgroundMedium = Color(0xFFA7A7A7),
    ripple = Color(0xFFECECEC),
    icons = Color(0xFFFFFFFF),
    primary = Color(0xFF0AA20F),
    primaryLight = Color(0xFF5DF863),
    text = Color(0xFFFFFFFF),
    textLight = Color(0xFFCCCCCC),
    expandedDialogBackground = Color(0x33000000),
    // endregion
)
